...
const sampleRequests = [
  { id: 1, title: "QF1 — Melbourne Airport", subtitle: "3 passengers, 4 luggage", date: "March 10, 2026", status: "Confirmed" },
  { id: 2, title: "SQ231 — Sydney Airport", subtitle: "1 passenger, 2 luggage", date: "March 18, 2026", status: "Pending" },
  { id: 3, title: "EK404 — Brisbane Airport", subtitle: "2 passengers, 3 luggage", date: "Feb 28, 2026", status: "Completed" },
];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Submit your airport pickup request</p>
      </div>
      <Tabs defaultValue="book" className="w-full">
        <TabsList>
          <TabsTrigger value="book">Book Request</TabsTrigger>
        ...
        <TabsContent value="book">
          {submitted ? (
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="mt-2 text-muted-foreground max-w-md">Your airport pickup request has been submitted. Track it in My Requests.</p>
              <Button className="mt-6" onClick={() => setSubmitted(false)}>Submit Another</Button>
            </div>
          ) : (
            <div className="rounded-xl border border-border bg-card p-6 shadow-card mt-4">
              <form onSubmit={(e) => { e.preventDefault(); setSubmitted(true); }} className="space-y-6">
                ...
                <FormSection title="Pickup Destination">
                  <div className="grid gap-4 sm:grid-cols-2">
                ...
                <FormSection title="Contact Information">
                  <div className="grid gap-4 sm:grid-cols-2">
                    <PhoneField value={form.primaryPhone} onChange={(v) => update("primaryPhone", v)} label="Primary Contact Number" />
                    <PhoneField value={form.whatsapp} onChange={(v) => update("whatsapp", v)} label="WhatsApp Number" />
                  </div>
                </FormSection>

                <Button type="submit" size="lg" className="w-full sm:w-auto">Submit Pickup Request</Button>
              </form>
            </div>
          )}
        </TabsContent>
        <TabsContent value="requests" className="mt-4">
          <RequestsListTab requests={sampleRequests} emptyMessage="No pickup requests yet." />
        </TabsContent>
      </Tabs>
    </div>
  );
